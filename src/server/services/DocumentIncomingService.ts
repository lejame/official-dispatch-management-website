...
@injectable()
export class DocumentIncomingService {
    ...
    public async getAllIncomingDocuments(): Promise<ResponseDTO<IDocument[] | null>> {
        try {
            const documents = await this.documentRepository.findAllWithPopulatedFields();
            const incomingDocuments = documents.filter((doc) => doc.type === 'incoming');
            return new ResponseDTO(200, 'Lấy danh sách công văn đến thành công', incomingDocuments, []);
        } catch (error) {
            return new ResponseDTO(500, 'Lấy danh sách công văn đến thất bại', null, [(error as Error).message]);
        }
    }

    public async createDocument(documentDTO: DocumentDTO): Promise<ResponseDTO<IDocument | null>> {
        const errors = this.validateDocument(documentDTO);

        if (errors.length > 0) {
            return new ResponseDTO(400, 'Tạo công văn thất bại', null, errors);
        }

        const document = new DocumentModel(documentDTO);
        const createdDocument = await this.documentRepository.create(document);

        return new ResponseDTO(201, 'Tạo công văn thành công', createdDocument, []);
    }
    ...
    public async updateDocumentStatus(id: string, status: string): Promise<ResponseDTO<IDocument | null>> {
        ...
        // Kiểm tra trạng thái được yêu cầu và cập nhật tương ứng
        if (status === 'approved') {
        ...
        return new ResponseDTO(200, 'Cập nhật trạng thái công văn thành công', updatedDocument, []);
    }
